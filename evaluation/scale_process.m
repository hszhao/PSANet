function data_output = scale_process(net,img_scale,fea_cha,crop_size,ori_rows,ori_cols,mean_rgb)
    data_output = zeros(ori_rows,ori_cols,fea_cha,'single');
    [new_rows, new_cols, ~] = size(img_scale);
    long_size = new_rows;
    short_size = new_cols;
    ...
    if(long_size <= crop_size)
        input_data = pre_img(img_scale,crop_size,mean_rgb);
        ...
        if(short_size < crop_size)
          if(new_rows < crop_size)
              im_r = padarray(img_pad(:,:,1),[crop_size-new_rows,0],mean_rgb(1),'post');
              im_g = padarray(img_pad(:,:,2),[crop_size-new_rows,0],mean_rgb(2),'post');
              im_b = padarray(img_pad(:,:,3),[crop_size-new_rows,0],mean_rgb(3),'post');
              img_pad = cat(3,im_r,im_g,im_b);
          end
          if(new_cols < crop_size)
              im_r = padarray(img_pad(:,:,1),[0,crop_size-new_cols],mean_rgb(1),'post');
              im_g = padarray(img_pad(:,:,2),[0,crop_size-new_cols],mean_rgb(2),'post');
              im_b = padarray(img_pad(:,:,3),[0,crop_size-new_cols],mean_rgb(3),'post');
              img_pad = cat(3,im_r,im_g,im_b);
          end
        end
        ...
        for grid_yidx=1:h_grid
            for grid_xidx=1:w_grid
                ...
                input_data = pre_img(img_sub,crop_size,mean_rgb);
